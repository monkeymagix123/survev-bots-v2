...
function buildSummary(fields: Record<string, unknown>): string {
    return [
        ...
        `emergency=${toPrintable(fields.emergencyState)}`,
        `eWhy=${toPrintable(fields.emergencyReason)}`,
        `macro=${toPrintable(fields.macroGoal)}`,
        `mWhy=${toPrintable(fields.macroReason)}`,
        `tactical=${toPrintable(fields.tacticalGoal)}`,
        `tWhy=${toPrintable(fields.tacticalReason)}`,
        `zone=${toPrintable(fields.targetZoneId)}`,
        `building=${toPrintable(fields.targetBuildingId)}`,
        `zoneScore=${toPrintable(fields.zoneScore)}`,
        `sub=${toPrintable(fields.subGoal)}`,
        `resume=${toPrintable(fields.resumeAfterSubGoal)}`,
        `loot=${toPrintable(fields.lootTargetId)}`,
        `obj=${toPrintable(fields.objectTargetId)}`,
        `zonePos=(${toPrintable(fields.targetZoneX)},${toPrintable(fields.targetZoneY)})`,
